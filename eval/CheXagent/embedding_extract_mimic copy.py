...
def run_gpu_worker(gpu_id, df_chunk, clean_label_cols):
    ...
    with torch.no_grad():
        ...
        dummy_output = model.get_image_features(**dummy_inputs).pooler_output
    ...
    with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.float16):
        for imgs_batch, indices in dataloader:
            ...
            outputs = model.get_image_features(pixel_values=valid_imgs)
            raw_outs = outputs.pooler_output  # Shape: [batch_size, EMBEDDING_DIM]
