...
def evaluate(
    ...
    for samples, targets, *_ in metric_logger.log_every(data_loader, 10, header):
        if multiview:
            outputs = [model.forward_(samples[0].to(device)), model.forward_(samples[1].to(device))]
        else:
...
def evaluate_preds(
    ...
    for samples, targets, *_ in metric_logger.log_every(data_loader, 10, header):
        ...
        for k, metric in metrics.items():  # only evaluate optimal classifier
            metric_inputs = postprocessors[k](outputs, targets)
            metric.update(**metric_inputs)

        preds.append(metric_inputs['preds'])
        target.append(metric_inputs['target'])
    ...
    return metric_logger_stats, stats, {"preds": preds, "target": target}
...
def make_data_loaders(train_dataset, test_dataset, val_dataset=None,
                    sampler_type=SamplerType.INFINITE, seed=0, start_iter=0,
    ...
    if val_dataset != None:
        val_data_loader = make_data_loader(
            dataset=val_dataset,
            batch_size=32,
            # num_workers=num_workers,  # didn't work for montgomery
            num_workers=0,
            sampler_type=None,
            drop_last=False,
            shuffle=False,
