...
from torchmetrics.classification import (MultilabelAUROC, MultilabelF1Score, MultilabelAccuracy, MulticlassF1Score, 
                                        ...
                                        JaccardIndex, MulticlassJaccardIndex, Dice, BinaryAUROC, MultilabelAveragePrecision,
                                        BinaryAccuracy)
...
class MetricType(Enum):
    ...
    CLASS_MULTILABEL_AUROC = "class_multilabel_auc"
    CLASS_MULTILABEL_AP = "class_multilabel_ap"
...
def build_metric(metric_type: MetricType, *, num_classes: int, labels = None, ks: Optional[tuple] = None):
    if metric_type == MetricType.MULTILABEL_ACCURACY:
    ...
    elif metric_type == MetricType.BINARY_ACCURACY:
        return build_binary_accuracy_metric(
    ...
    elif metric_type == MetricType.CLASS_MULTILABEL_AUROC:
        return build_class_multilabel_auroc_metric(
            num_labels=num_classes,
            labels=labels
        )
    elif metric_type == MetricType.MULTILABEL_AP:
        return build_multilabel_ap_metric(
            average_type=metric_type.accuracy_averaging,
            num_labels=num_classes,
        )
    elif metric_type == MetricType.CLASS_MULTILABEL_AP:
        return build_class_multilabel_ap_metric(
            num_labels=num_classes
        )
        ...
        print(metric_type==MetricType.BINARY_AUROC)
...
def build_binary_accuracy_metric():
    metrics: Dict[str, Metric] = {
         f"accuracy": BinaryAccuracy()
    }
    return MetricCollection(metrics)
...
def build_class_multilabel_auroc_metric(num_labels: int, labels=None):
    metrics: Dict[str, Metric] = {
        # "class-specific": MetricCollection({
            "class_auroc": ClasswiseWrapper(MultilabelAUROC(num_labels=num_labels, average=None), labels=labels, prefix=" "),
        # })
    }
    return MetricCollection(metrics)


def build_multilabel_ap_metric(average_type: MetricAveraging, num_labels: int):
    metrics: Dict[str, Metric] = {
        "ap": MultilabelAveragePrecision(num_labels=num_labels, average=average_type.value),
        "auroc": MultilabelAUROC(num_labels=num_labels, average=average_type.value),
    }
    return MetricCollection(metrics)


def build_class_multilabel_ap_metric(num_labels: int):
    metrics: Dict[str, Metric] = {
        "ap": MultilabelAveragePrecision(num_labels=num_labels, average=None),
