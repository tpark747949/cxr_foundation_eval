...
def main():
    ...
    # Open pre-computed prediction tables for native inferences
    tbl_chexagent = db_txt.open_table("CheXagent")
    tbl_biovil = db_txt.open_table("BioViL-T")
    
    print("Fetching text embeddings and pre-computed predictions...")
    df_txt = tbl_txt.search().to_pandas()
    txt_lookup = {row["disease"]: row for _, row in df_txt.iterrows()}
    
    # Create fast dictionary lookups mapping dicom_id -> prediction struct
    print("Building dictionary lookups for pre-computed models...")
    ...
    for model in MODELS:
        ...
        # --- EXCEPTION PATH: Pre-computed Models ---
        if model in ["CheXagent", "BioViL-T"]:
            ...
            for disease in DISEASES:
                ...
                for d_id in results["dicom_id"]:
                    # Fetch the precomputed 0 or 1, defaulting to NaN if missing
                    if d_id in lookup and lookup[d_id] is not None:
                        scores.append(lookup[d_id].get(struct_key, np.nan))
                    else:
                        scores.append(np.nan)
                        
                results[f"score_{model}_{struct_key}"] = scores
            continue # Skip the embedding dot-product logic for these models

        # --- STANDARD PATH: Computed on the fly via Embeddings ---
        img_col = f"{model}_l2" 
